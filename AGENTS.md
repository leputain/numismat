...
- Production HTTP auth requires one canonical HTTPS `MINIAPP_PUBLIC_URL`, an independent 32-byte `HTTP_SECURITY_KEY`, host-only Secure cookies, and double-submit CSRF. Initial signed Telegram login treats a single bounded ASCII Origin as optional transport metadata because native WebViews do not provide one portable Origin contract; verified Telegram HMAC, exact owner, TTL, and replay denial remain authoritative. Logout and every protected mutation always require exact Origin. Never add an auth bypass or move session validation into a transaction separate from the protected mutation.
