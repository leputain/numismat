...
- Production HTTP auth requires one canonical HTTPS `MINIAPP_PUBLIC_URL`, an independent 32-byte `HTTP_SECURITY_KEY`, host-only Secure cookies, and double-submit CSRF. Signed login, logout, and protected mutations treat a single bounded ASCII Origin as optional transport metadata because native WebViews do not provide one portable Origin contract; duplicate, empty, oversized, and non-ASCII values fail closed. Verified Telegram HMAC, exact owner, TTL, and replay denial authorize login; the host-only session plus double-submit CSRF authorize logout and protected writes, with idempotency required for mutations. Never add an auth bypass or move session validation into a transaction separate from the protected mutation.
