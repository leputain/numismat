...
- Production HTTP auth requires one canonical HTTPS `MINIAPP_PUBLIC_URL`, an independent 32-byte `HTTP_SECURITY_KEY`, exact Origin, host-only Secure cookies, and double-submit CSRF. Never add an auth bypass or move session validation into a transaction separate from the protected mutation.
