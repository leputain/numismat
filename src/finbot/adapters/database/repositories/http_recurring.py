...
class SqlAlchemyRecurringQueryUnitOfWork:
    """One authenticated, read-only repeatable snapshot for recurring reads."""

    __slots__ = ("_context", "_sessions", "auth", "recurring", "session")
    ...
    def __init__(self, sessions: async_sessionmaker[AsyncSession]) -> None:
        ...
        self.recurring: RecurringReader

    async def __aenter__(self) -> SqlAlchemyRecurringQueryUnitOfWork:
        ...
        try:
            ...
            self.recurring = SqlAlchemyRecurringRepository(self.session)
...
class SqlAlchemyRecurringQueryUnitOfWorkFactory:
    __slots__ = ("_sessions",)

    def __init__(self, sessions: async_sessionmaker[AsyncSession]) -> None:
        self._sessions = sessions

    def __call__(self) -> SqlAlchemyRecurringQueryUnitOfWork:
        return SqlAlchemyRecurringQueryUnitOfWork(self._sessions)


__all__ = [
    "SqlAlchemyRecurringQueryUnitOfWork",
